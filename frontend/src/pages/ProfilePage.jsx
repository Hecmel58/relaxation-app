...
function ProfilePage() {
  ...
  const handleDownloadData = async () => {
    try {
      ...
      if (!response.ok) throw new Error('Veri indirme başarısız');
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <p className="text-slate-900">
              ...
              {user?.abGroup === 'experiment' && <span className="ml-2 px-2 py-1 bg-warning-100 text-warning-800 rounded text-xs font-medium">BETA</span>}
            </p>
          </div>
        </div>
      </Card>
      <Card>
        ...
        <p className="text-sm text-slate-600 mb-4">6698 sayılı KVKK kapsamında kişisel verilerinizi indirebilir veya hesabınızı silebilirsiniz.</p>
        <div className="space-y-3">
          <button onClick={handleDownloadData} disabled={downloading} className="w-full bg-blue-500 hover:bg-blue-600 disabled:bg-blue-300 text-white font-semibold py-3 px-4 rounded-lg transition-colors">
            {downloading ? 'İndiriliyor...' : 'Verilerimi İndir (JSON)'}
          </button>
          <button onClick={handleDeleteAccount} className="w-full bg-red-500 hover:bg-red-600 text-white font-semibold py-3 px-4 rounded-lg transition-colors">
            Hesabımı Kalıcı Olarak Sil
          </button>
          <a href="/privacy-policy" target="_blank" rel="noopener noreferrer" className="block text-center text-blue-600 hover:text-blue-700 hover:underline py-2">
            Gizlilik Politikası ve KVKK Aydınlatma Metni
          </a>
          <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
            <p className="text-xs text-amber-800"><strong>Önemli:</strong> Hesabınızı sildiğinizde tüm verileriniz kalıcı olarak silinecektir. Bu işlem geri alınamaz.</p>
          </div>
        </div>
      </Card>
    </div>
