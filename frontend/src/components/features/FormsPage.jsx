        ...
        {forms.map((form) => (
          <Card key={form.id} className="relative hover:shadow-lg transition-shadow">
            ...
            <div className="flex gap-3 mt-4">
              <a href={form.google_form_url} target="_blank" rel="noopener noreferrer" className={`flex-1 inline-flex items-center justify-center px-4 py-2 text-sm font-medium rounded-md text-white ${form.is_filled ? 'bg-slate-400 hover:bg-slate-500' : 'bg-primary-600 hover:bg-primary-700'}`}>
              ...
              {form.is_filled && (
                <button onClick={handleRefillForm} className="px-4 py-2 border-2 border-primary-600 text-primary-600 text-sm font-medium rounded-md hover:bg-primary-50" title="Yeniden doldur">
